...
public class EditorWheelController : MonoBehaviour, EventHandler{
	...
	float rotScale = 1.5f; //speeds up or slows down the rotation. should probably stay at 1.0, unless playtesting discovers otherwise
	...
	public void HandleEvent(GameEvent ge){
		...
		if (ge.type.Equals ("mouse_release")) {
			...
			if(spinner == false){
				float dist = Mathf.Sqrt((mousepos.x*mousepos.x)+(mousepos.y*mousepos.y));
	...
	void Update () {
		...
		if(touchDown){
			...
			if(spinner && clickTime > clickDelay){
				...
				PieceController pc = EditorController.GetFloatingPiece();
				if(pc != null)
					pc.transform.rotation = Quaternion.Euler(0, 0, transform.rotation.eulerAngles.z);
			}
		}
	}*/
